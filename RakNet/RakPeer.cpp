...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		if ((unsigned char)packet->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) {
			...
			BYTE byteNameLen = (BYTE)strlen(playerName);
			BYTE iClientVerLen = (BYTE)strlen(version);
			...
			bsSend.Write(playerName, byteNameLen);
			bsSend.Write(uiClientChallengeResponse);
			bsSend.Write(byteAuthBSLen);
			bsSend.Write(auth_bs, byteAuthBSLen);
			bsSend.Write(iClientVerLen);
			bsSend.Write(version, iClientVerLen);

			RPC(&RPC_ClientJoin, &bsSend, HIGH_PRIORITY, RELIABLE, 0, remoteSystemList[0].playerId, false, FALSE, UNASSIGNED_NETWORK_ID, NULL);
			delete packet->data;

			BYTE *data = new BYTE[3];
			RakNet::BitStream bsConnNotify(data, sizeof(BYTE) * 3, false);
			bsConnNotify.Reset();
			bsConnNotify.Write((BYTE)ID_CONNECTED_NOTIFY);
			bsConnNotify.Write((WORD)myPlayerID);

			packet = (Packet *)malloc(sizeof(Packet));
			packet->data = data;
			packet->length = sizeof(BYTE) * 3;
			packet->deleteData = true;
			packet->bitSize = (sizeof(BYTE) * 3) * 8;
		}
		if ((unsigned char)packet->data[0] == ID_AUTH_KEY) {
